...
class PurchasesTest extends PHPUnit_Framework_TestCase
{
	...
	public function testCRUD_Ok()
	{
		...
		TestEnvironment::createSupplier($client, 'TEST_SUPPLIER', 'Test Supplier');
		TestEnvironment::createItem($client, 'TEST_ITEM', 'Test Item'); // Does this need to change for supply? CP 2015-09

		// List
		$response = $client->get('/modules/api/sales/' . ST_SUPPINVOICE, array(
		...
		$response = $client->post('/modules/api/purchases/', array(
			'headers' => TestEnvironment::headers(),
			'body' => array(
		...
		// List again
		$response = $client->get('/modules/api/purchases/' . ST_SUPPINVOICE .'/', array(
			'headers' => TestEnvironment::headers()
		));
		$this->assertEquals('200', $response->getStatusCode());
		$result = $response->getBody();
		...
		// Get by id
		$response = $client->get('/modules/api/purchases/' . $id . '/' . ST_SUPPINVOICE, array(
			'headers' => TestEnvironment::headers()
		));
		$this->assertEquals('200', $response->getStatusCode());
		$result = $response->getBody();
		...
		// Write back
		$response = $client->put('/modules/api/purchases/' . $id . '/' . ST_SUPPINVOICE, array(
			'headers' => TestEnvironment::headers(),
			'body' => array(
